...
class SignalFactChecker:
    ...
    def bulk_fact_check_live_signals_parallel(self, symbol: str = None,
        ...
        logging.info(f"✅ Done: {results['total_checked']} checks in {elapsed:.1f}s ({results['total_checked']/elapsed:.1f}/s)")
        logging.info(f"   Accuracy: {results['accuracy']:.1f}%")
        logging.info(f"   Profit Factor: {results['profit_factor']:.2f}")
        logging.info(f"   Stopped: {results['stopped_out']} ({results['stopped_out']/max(results['total_checked'],1)*100:.1f}%)")
    ...
    def bulk_fact_check_position_signals_parallel(self, symbol: str = None,
        ...
        logging.info(f"✅ Position signals: {results['total_checked']} checks in {elapsed:.1f}s ({results['total_checked']/elapsed:.1f}/s)")
        logging.info(f"   Accuracy: {results['accuracy']:.1f}%")
        logging.info(f"   Profit Factor: {results['profit_factor']:.2f}")
    ...
    def bulk_fact_check_all_signals(self, symbol: str = None,
        ...
        logging.info(f"Position Signals: {position_results['total_checked']} checked, {position_results['accuracy']:.1f}% accuracy")
