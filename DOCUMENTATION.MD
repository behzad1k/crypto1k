...
ok now I need a comprehensive paper trading system. I need you to add a whole new system that accepts an initial bankroll and  checks the signals generated by my monitoring system. if the signals passes my requirements, the coin is added to a watch-list in buying queue. for example if the signals detected at 1.512 usdt the system adds it to the watchlist that waits for the price to drop by 1 percent (should be a variable: MIN_BUYING_WINDOW_PCT) then attempts to buy it, so we dont buy the coin while peaking. if no buying window within the next 10 minutes (should be a variable: MAX_BUYING_WINDOW_TIME) the coins is removed from buying watchlist. after buying it the coin will still get monitored for a selling opportunity which is when it exceeds MAX_PROFIT_THRESHOLD_PCT, unless STOP_LOSS_PCT is met and we sell the coin. 

while watching the bought coin, you should apply the my signals on them and save the results in the live_signals table without duplicating the timeframes, 

you should consider the exchange fee for the trading which is about 0.1 percent (EXCHANGE_FEE). I want a whole frontend page to show me all the positions, and either another page for each position or a modal describing exactly whatever happened to them and a summary of what is being done to each position. remove my previous trading_positions and position_signals.

also here is my frontend symbol page as a reference for the new positions page.