...
def _bucket_stats(rows: list, min_n: int) -> dict:
    ...
    ret_1h = [r['ret_1h'] for r in rows if r['ret_1h'] is not None]
    ...
    return {
        'n': n,
        'avg_ret_1h_pct':      round(mean(ret_1h), 2) if ret_1h else None,
        'avg_ret_24h_pct':     round(mean(ret_24h), 2) if ret_24h else None,
