...
from crypto1k.config.signal_config import (
    CATEGORY_ORDER,
    GRADE_WEIGHTS,
    HORIZONS,
    SIGNAL_FAMILIES,
...
@app.route("/api/scanner/backfill-outcomes", methods=["POST"])
@login_required
def scanner_backfill_outcomes():
    """Fetch forward price outcomes for alerts that don't have one yet."""
    data = request.get_json() or {}
    limit = int(data.get("limit", 200))
    result = outcomes.backfill_outcomes(limit=limit)
    return jsonify({"success": True, **result})


@app.route("/api/scanner/evaluation")
@login_required
def scanner_evaluation():
    """Hit rate / avg forward return per score band, alert path, and symbol."""
    min_bucket = int(request.args.get("min_bucket", 3))
    report = evaluation.generate_report(min_alerts_per_bucket=min_bucket)
    return jsonify({"success": True, **_sanitize(report)})
